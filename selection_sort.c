...
void selection_sort(long long *arr, long long n, Metrics *metrics) {
    // Initialize metrics
...
// Function to generate sample sizes (same as in sample_generator.c)
...
int main() {
    ...
    FILE *csv_file = fopen("metrics/selection_sort_metrics.csv", "w");
    if (csv_file == NULL) {
        perror("fopen");
        exit(EXIT_FAILURE);
    }
    ...
    for (int idx = 0; idx < NUM_SIZES; idx++) {
        ...
        for (int dtype = 0; dtype < num_data_types; dtype++) {
            ...
            for (int run = 1; run <= NUM_RUNS; run++) {
                ...
                Metrics metrics;
                selection_sort(data, data_size, &metrics);
    ...
    printf("Metrics have been exported to metrics/selection_sort_metrics.csv\n");
